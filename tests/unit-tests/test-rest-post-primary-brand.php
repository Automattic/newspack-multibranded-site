 ...
 */
class Test_Rest_Post_Primary_Brand extends WP_UnitTestCase {
	 ...
	 */
	private function distpatch_request_to_edit_the_postmeta( $value ) {
		$endpoint = '/wp/v2/posts/' . $this->post->ID;
		...
		$request->set_body(
			wp_json_encode(
				[
					'meta' => [
						Taxonomy::PRIMARY_META_KEY => $value,
					],
				]
			)
		);
	...
	public function test_unauthorized() {
		...
		$response = $this->distpatch_request_to_edit_the_postmeta( 123 );
	...
	public function test_invalid_input() {
		wp_set_current_user( $this->user_id->ID );
		$response = $this->distpatch_request_to_edit_the_postmeta( 'invalid' );
		$data     = $response->get_data();
